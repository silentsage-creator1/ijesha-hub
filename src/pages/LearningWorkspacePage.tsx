...
export function LearningWorkspacePage({ kind }: { kind: Kind | 'grading' | 'results' }) {
  ...
  const grade = async (submission: Submission) => {
    const result = await fetch(apiUrl(`/api/learning/submissions/${submission.id}/grade`), { method: 'PATCH', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ score, feedback }) })
