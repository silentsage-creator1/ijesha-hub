...
type Kind = 'classwork' | 'assessment' | 'project'
...
const label = (kind: Kind) => kind === 'classwork' ? 'Classwork' : kind === 'assessment' ? 'Assessment' : 'Project'
...
export function LearningWorkspacePage({ kind }: { kind: Kind | 'grading' | 'results' }) {
  ...
  const load = async () => {
    const result = await fetch('/api/learning/items', { credentials: 'include' })
    const data = await result.json().catch(() => ({}))
    if (!result.ok) { setError(data.error || 'Unable to load learning work.'); return }
    setItems(data.items ?? [])
    if (staff) {
      const cohortResult = await fetch('/api/learning/cohorts', { credentials: 'include' })
  ...
  const create = async () => {
    ...
    const result = await fetch('/api/learning/items', { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ itemType: kind, title, instructions, cohortId, dueAt, maximumScore }) })
    const data = await result.json().catch(() => ({}))
  ...
  const openItem = async (item: Item) => {
    setSelected(item); setResponse(''); setScore(''); setFeedback(''); setSubmissions([])
    if (staff) { const result = await fetch(`/api/learning/items/${item.id}/submissions`, { credentials: 'include' }); const data = await result.json().catch(() => ({})); if (result.ok) setSubmissions(data.submissions ?? []); else setError(data.error || 'Unable to load submissions.') }
  }
  const submit = async () => {
    if (!selected || !response.trim()) return
    const result = await fetch(`/api/learning/items/${selected.id}/submissions`, { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ response }) })
  ...
  const grade = async (submission: Submission) => {
    const result = await fetch(`/api/learning/submissions/${submission.id}/grade`, { method: 'PATCH', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ score, feedback }) })
    const data = await result.json().catch(() => ({})); if (!result.ok) setError(data.error || 'Unable to save grade.'); else await openItem(selected!)
  }
  return <div className="space-y-6"><PageHeader title={heading} subtitle={kind === 'results' ? 'Grades are created from evaluated student submissions.' : staff ? 'Create work, review submissions, and keep learning records shared across accounts.' : 'View your assigned learning work and submit your response.'} actions={staff && !['grading', 'results'].includes(kind) ? <button onClick={() => setCreateOpen(true)} className="inline-flex items-center gap-2 rounded-[var(--radius-md)] bg-[var(--color-harbor-500)] px-4 py-2 text-sm font-semibold text-white"><Plus size={16}/>Add {label(kind as Kind)}</button> : undefined}/>{error && <p className="text-sm text-[var(--color-danger-600)]">{error}</p>}<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map((item) => <Card key={item.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><Badge tone={item.status === 'published' ? 'success' : 'warning'}>{label(item.item_type)}</Badge><h2 className="mt-3 font-display text-lg font-bold">{item.title}</h2></div><span className="text-sm font-semibold">{item.maximum_score} pts</span></div><p className="mt-2 text-sm text-[var(--color-ink-600)]">{item.instructions || 'No instructions provided.'}</p><p className="mt-3 text-xs text-[var(--color-ink-500)]">{item.cohorts?.name || '—'} · Due {item.due_at ? new Date(item.due_at).toLocaleDateString() : 'Not set'}</p><button onClick={() => void openItem(item)} className="mt-4 text-sm font-semibold text-[var(--color-harbor-700)]">{staff ? 'View submissions' : 'Open & submit'} →</button></Card>)}</div>{visible.length === 0 && <Card className="p-10 text-center text-sm text-[var(--color-ink-500)]">No {kind === 'results' ? 'graded results' : 'learning work'} yet.</Card>}{createOpen && <Modal title={`Add ${label(kind as Kind)}`} onClose={() => setCreateOpen(false)}><div className="space-y-4"><Field label="Title"><input className="input" value={title} onChange={(event) => setTitle(event.target.value)} /></Field><Field label="Cohort"><select className="input" value={cohortId} onChange={(event) => setCohortId(event.target.value)}><option value="">Select a cohort</option>{cohorts.map((cohort) => <option key={cohort.id} value={cohort.id}>{cohort.name} ({cohort.courseName})</option>)}</select></Field><Field label="Instructions"><textarea className="input min-h-24" value={instructions} onChange={(event) => setInstructions(event.target.value)} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Due date"><input className="input" type="date" value={dueAt} onChange={(event) => setDueAt(event.target.value)} /></Field><Field label="Maximum score"><input className="input" type="number" min="1" value={maximumScore} onChange={(event) => setMaximumScore(event.target.value)} /></Field></div><button disabled={!title.trim() || !cohortId} onClick={() => void create()} className="rounded-[var(--radius-md)] bg-[var(--color-harbor-500)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Publish</button></div></Modal>}{selected && <Modal title={selected.title} onClose={() => setSelected(null)}>{staff ? <div className="space-y-4">{submissions.length ? submissions.map((submission) => <Card key={submission.id} className="p-4"><p className="font-semibold">{submission.students?.full_name || 'Student'}</p><p className="mt-2 text-sm text-[var(--color-ink-600)]">{submission.response}</p><p className="mt-2 text-xs text-[var(--color-ink-500)]">{submission.submitted_at ? `Submitted ${new Date(submission.submitted_at).toLocaleString()}` : 'Draft'}</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><input className="input" type="number" placeholder={`Score / ${selected.maximum_score}`} value={score} onChange={(event) => setScore(event.target.value)} /><input className="input" placeholder="Feedback" value={feedback} onChange={(event) => setFeedback(event.target.value)} /></div><button onClick={() => void grade(submission)} className="mt-2 rounded-[var(--radius-md)] bg-[var(--color-harbor-500)] px-3 py-1.5 text-xs font-semibold text-white">Save grade</button></Card>) : <p className="text-sm text-[var(--color-ink-500)]">No student submissions yet.</p>}</div> : <div className="space-y-4"><p className="text-sm text-[var(--color-ink-600)]">{selected.instructions}</p><Field label="Your response"><textarea className="input min-h-32" value={response} onChange={(event) => setResponse(event.target.value)} /></Field><button disabled={!response.trim()} onClick={() => void submit()} className="rounded-[var(--radius-md)] bg-[var(--color-harbor-500)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Submit work</button></div>}</Modal>}</div>
}
