...
export function AppCoursesPage() {
  ...
  async function createCourse(event: FormEvent) {
    ...
    try {
      ...
      setOpen(false)
      navigate(`/courses/${payload.course.id}`)
    } catch (saveError) {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <Card className="p-6 text-sm text-[var(--color-ink-500)]">Loading courses…</Card> : courses.length === 0 ? <Card className="p-8 text-center"><BookOpen className="mx-auto mb-3 text-[var(--color-ink-400)]" size={30} /><p className="font-semibold text-[var(--color-ink-900)]">No courses yet</p><p className="mt-1 text-sm text-[var(--color-ink-500)]">Courses will appear here when they are created and assigned.</p></Card> : <div className="grid gap-4 md:grid-cols-2">{courses.map((course) => <div key={course.id} className="relative"><button type="button" onClick={() => navigate(`/courses/${course.id}`)} aria-label={`Open ${course.title}`} className="w-full cursor-pointer rounded-lg text-left focus-visible:outline-2 focus-visible:outline-[var(--color-harbor-600)]"><Card className="h-full p-5 transition-shadow hover:shadow-[var(--shadow-pop)]"><div className="flex items-start justify-between gap-3"><div><h2 className="font-display text-lg font-bold text-[var(--color-ink-900)]">{course.title}</h2><p className="mt-1 text-sm text-[var(--color-ink-600)]">{course.description || 'No description provided.'}</p></div><Badge tone={course.status === 'Published' ? 'success' : 'neutral'}>{course.status}</Badge></div><div className="mt-4 flex gap-4 text-xs text-[var(--color-ink-500)]"><span>{course.level || 'Level not set'}</span><span className="inline-flex items-center gap-1"><Users size={13} /> {course.cohort_id ? 'Cohort assigned' : 'No cohort assigned'}</span></div><p className="mt-4 text-sm font-semibold text-[var(--color-harbor-700)]">Open Course →</p></Card></button>{canManage && <button type="button" aria-label={`Delete ${course.title}`} onClick={() => { setDeleteError(''); setDeleteTarget(course) }} className="mt-2 rounded-lg border border-[var(--color-line)] px-3 py-2 text-sm font-semibold text-[var(--color-danger-600)]">Delete course</button>}</div>)}</div>}
