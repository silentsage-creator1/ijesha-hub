...
import { trackCourse } from '@/lib/trackCourse'
...
export function StudentsPage() {
  ...
  const [moveCohort, setMoveCohort] = useState('')
  const [moving, setMoving] = useState(false)
  const [moveMessage, setMoveMessage] = useState('')
  async function moveSelected() {
    if (!selected.length || !moveCourse || !validMoveCohort || moving) return
    if (!window.confirm(`Assign ${selected.length} selected students to ${cohorts.find(c=>c.id===moveCohort)?.name}? Their history will be preserved.`)) return
    setMoving(true); setMoveMessage('')
    try {
      const result = await cohortRequest<{results:{studentId:string;error:string|null}[]}>('/api/students/assign-cohort',{method:'POST',body:JSON.stringify({studentIds:selected,cohortId:moveCohort})})
      const failures = result.results.filter(r=>r.error)
      setSelected(failures.map(r=>r.studentId))
      setMoveMessage(`${result.results.length-failures.length} assigned. ${failures.length} failed.` + failures.map(r=>` ${students.find(s=>s.id===r.studentId)?.full_name ?? 'Student'}: ${r.error}`).join(''))
      await load()
    } catch(err) { setMoveMessage(err instanceof Error ? err.message : 'Unable to assign students.') }
  ...
  const selectedTracks = new Set(selectedStudents.map(student => trackCourse(student.track)))
  const matchingCourses = selectedTracks.size === 1 && !selectedTracks.has('')
    ? courses.filter(course => selectedTracks.has(trackCourse(course.name))) : []
  const moveCourse = matchingCourses.length === 1 ? matchingCourses[0].id : ''
  const eligibleCohorts = cohorts.filter(c => c.app_course_id === moveCourse && c.status !== 'Completed')
  const validMoveCohort = eligibleCohorts.some(c => c.id === moveCohort)
  useEffect(() => { setMoveCohort('') }, [moveCourse])
