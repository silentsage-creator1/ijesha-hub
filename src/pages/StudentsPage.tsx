...
export function StudentsPage() {
 ...
 const visible=useMemo(()=>students.filter(student=>{const q=query.toLowerCase();return (!q||student.full_name.toLowerCase().includes(q)||(student.email??'').toLowerCase().includes(q)||(student.cohort??'').toLowerCase().includes(q))&&(!statusFilter||student.status===statusFilter)&&(!courseFilter||student.track===courseFilter)}),[students,query,statusFilter,courseFilter]);
  const available = cohorts.filter(c=>c.app_course_id===form.courseId)
