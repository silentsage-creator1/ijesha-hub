...
export function StudentsPage() {
  ...
  async function moveSelected() {
    if (!selected.length || !moveCohort || moving) return
 ...
 return <div><PageHeader title="Students" subtitle={canManage?'Search the roster or create a new student account.':'Students assigned to you.'} actions={canManage?<button onClick={()=>setOpen(true)} className="flex items-center gap-1.5 rounded-[var(--radius-md)] bg-[var(--color-harbor-500)] px-3.5 py-2 text-sm font-semibold text-white"><Plus size={16}/>Add Student</button>:undefined}/><div className="mb-4 flex flex-wrap gap-3"><div className="relative max-w-sm flex-1 min-w-[200px]"><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-ink-400)]"/><input className="input pl-9" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search students"/></div><select className="input w-auto min-w-[130px]" value={statusFilter} onChange={event=>setStatusFilter(event.target.value)}><option value="">All statuses</option><option value="new">New</option><option value="active">Active</option><option value="paused">Paused</option><option value="graduated">Graduated</option><option value="withdrawn">Withdrawn</option></select><select className="input w-auto min-w-[130px]" value={courseFilter} onChange={event=>setCourseFilter(event.target.value)}><option value="">All tracks</option>{Array.from(new Set([...courses.map(c=>c.name),...students.map(s=>s.track).filter((t):t is string=>Boolean(t))])).map(name=><option key={name} value={name}>{name}</option>)}</select></div>{error&&!open&&<p className="mb-4 rounded-md bg-[var(--color-danger-100)] p-3 text-sm text-[var(--color-danger-600)]">{error}</p>}{role==='admin'&&<Card className="mb-4 space-y-3 p-4">
    ...
    <div className="flex flex-wrap gap-2"><select aria-label="Destination course" disabled={moving} className="input w-auto" value={moveCourse} onChange={e=>{setMoveCourse(e.target.value);setMoveCohort('')}}><option value="">Select destination course</option>{courses.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>
    ...
    <button disabled={moving||!selected.length||!moveCohort} onClick={()=>void moveSelected()} className="rounded-lg bg-[var(--color-harbor-600)] px-4 py-2 text-sm text-white disabled:opacity-50">{moving?'Assigning…':`Assign selected (${selected.length})`}</button><button disabled={moving} onClick={()=>setSelected([])} className="text-sm">Clear selection</button></div>
