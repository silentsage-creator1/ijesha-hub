...
export function AppCourseDetailPage() {
  ...
  return <div className="space-y-6"><Link to="/courses" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-harbor-700)]"><ArrowLeft size={16} /> Back to courses</Link><PageHeader title={course.title} subtitle={course.description || 'No course description provided.'} actions={canManage ? <button type="button" onClick={() => { setForm({ title: course.title, description: course.description ?? '', status: course.status }); setSaveError(''); setMessage(''); setEditing(true) }} className="rounded-lg bg-[var(--color-harbor-600)] px-4 py-2 text-sm font-semibold text-white">Edit Course</button> : undefined} />
